	...
	function class_sms()
	{
		...
		if (isset($_POST['submit'])) 
		{
			...
			//foreach($check_group as $key => $value) {
				...
				// print_r($stdnt_list);
               //	$result=array_chunk($stdnt_list,30);			
					...
					$data = array();
					  //if(preg_match('/^\d{10}$/', $numbers)) { // phone number is valid
						...
						if ($save['sms_type'] != 'Schedule') {
							...
							if($data) {						
							$mobile_number[$i_key] = $numbers;
							$json = array();
							$store_data[$i_key] = $save;
							$data_api['numbers'] = $numbers;				
		                    $json = send_sms($data_api, $save);	
			...
			if($j_error==0) {
				$this->session->set_flashdata('success', '('.$i.') Message send succesfully');
				redirect($this->config->item('admin_folder') . '/send/class_sms');
			}
			else {
