...
<!--                            <a class="btn btn-default pull-left" target="_blank" href="<?php echo base_url().$this->config->item('admin_folder')?>/staff/import">Import Excel</a>-->
                             <a class="btn btn-default pull-right" href="<?php echo base_url().$this->config->item('admin_folder')?>/staff/mode">Add Staff</a>
                ...
                <div class="box-body">
                    ...
                    <table id="example2" class="table table-bordered table-striped">
                      <thead>
                        ...
                        <?php } else { ?>
                        <form name="list_form" action="<?php echo base_url().$this->config->item('admin_folder')?>/staff/mul_action" method="POST" >
                            ...
                            foreach ($data as $data_list){ ?>
                                <tr>  
                                    ...
                                    if($data_list['status']=='Active'){
                                        $checked = 'checked'; ?>  
                                        <input type='checkbox' name='boot_btn' id='<?php echo $data_list['id'];?>' value='/staff/change_status/' class='form-control cls_chk'  data-size='mini' <?php echo $checked ?> >
                                    <?php } else { ?>
                                        <input type='checkbox' name='boot_btn' id='<?php echo $data_list['id'];?>' value='/staff/change_status/' class='form-control cls_chk'  data-size='mini'></td>
                                    <?php } ?>
                                    <td><a class="pencil-square-o" title="Edit" href="<?php echo base_url().$this->config->item('admin_folder').'/staff/mode/'.$data_list['id']?>"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;&nbsp;
